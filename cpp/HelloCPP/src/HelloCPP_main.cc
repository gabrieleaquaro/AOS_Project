...
#define BBQUE_LOG_MODULE "HelloCPP"
...
/**
 * The decription of each HelloCPP parameters
 */
po::options_description opts_desc("HelloCPP Configuration Options");

/**
 * The map of all HelloCPP parameters values
 */
 ...
 */
std::string conf_file = BBQUE_PATH_PREFIX "/" BBQUE_PATH_CONF "/HelloCPP.conf" ;
...
void ParseCommandLine(int argc, char *argv[]) {
	...
	if (opts_vm.count("version")) {
		std::cout << "HelloCPP (ver. " << g_git_version << ")\n";
...
int main(int argc, char *argv[]) {
	...
	opts_desc.add_options()
		...
		("conf,C", po::value<std::string>(&conf_file)->
			...
			"HelloCPP configuration file")

		("recipe,r", po::value<std::string>(&recipe)->
			default_value("HelloCPP"),
			"recipe name (for all EXCs)")
	;

	// Setup a logger
	bu::Logger::SetConfigurationFile(conf_file);
	logger = bu::Logger::GetLogger("hellocpp");

	ParseCommandLine(argc, argv);

	// Welcome screen
	logger->Info(".:: HelloCPP (ver. %s) ::.", g_git_version);
	...
	pexc = std::make_shared<HelloCPP>("HelloCPP", recipe, rtlib);
